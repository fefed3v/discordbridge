...
namespace DiscordBridge
{
    ...
    HttpResponse HttpClient::request(const wchar_t* method, const std::wstring& host, const std::wstring& path, const std::wstring& headers, const std::string& body)
    {
        ...
        HINTERNET session = WinHttpOpen(USER_AGENT, WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
        ...
        requestHandle = WinHttpOpenRequest(connection, method, path.c_str(), nullptr, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, WINHTTP_FLAG_SECURE);
        if (!requestHandle)
        {
        ...
        if (!WinHttpSendRequest(requestHandle, headerData, headerSize, requestData, requestDataSize, requestDataSize, 0) || !WinHttpReceiveResponse(requestHandle, nullptr))
        {
        ...
        while (true)
        {
            ...
            DWORD bytesRead = 0;
            if (!WinHttpReadData(requestHandle, buffer.data(), available, &bytesRead) || bytesRead == 0) break;
